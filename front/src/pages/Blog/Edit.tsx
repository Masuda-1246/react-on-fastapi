...
import { useGetBlogPostApiBlogPostIdGet, useUpdateBlogPostApiBlogPostIdPut } from '../../api/generated/fastAPIReactBackend';
...
const BlogEditPage: FC = () => {
  ...
  const { data: blogPost, isLoading, error } = useGetBlogPostApiBlogPostIdGet(id || '');

  // useEffectを使用してblogPostが変更されたときにフォームデータを更新
  useEffect(() => {
    if (blogPost) {
      setFormData({
  ...
  // updateMutationを使用してブログ投稿を作成
  const updateMutation = useUpdateBlogPostApiBlogPostIdPut(
    {
      mutation: {
        onSuccess: () => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">ブログ投稿編集</h1>
          <a 
            href="/blog" 
